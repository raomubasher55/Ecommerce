...
const CategoryProducts = () => {
  const { categoryProducts, category, images } = useProduct();
  const navigate = useNavigate();

  return (
    <Layout>
      <div className='pt-24'>
        <div className='text-center' ><h1 className='font-bold text-2xl' > {category.map((c, index) => (c.name))} </h1></div>
        <div className='text-center' ><h1 className=' text-lg' > Products : {categoryProducts.length} </h1></div>
        <div className='flex mx-12 gap-8 justify-center  my-4'>
          {categoryProducts.length >= 1 ? categoryProducts.map((product, index) => (
